...
const ProductDisplayPage = () => {
  ...
  const fetchRelatedProducts = async (categoryId, categoryName, subCategoryId = null, subCategoryName = null, productName = '') => {
    ...
    try {
      ...
      if (normCatId && normSubId) {
        ...
        const res = await Axios.post('http://localhost:5173/api/product/get-pruduct-by-category-and-subcategory', {
      ...
      if (normCatId) {
        ...
        const res = await Axios.post('http://localhost:5173/api/product/get-product-by-category', {
          id: normCatId,
          limit: 24
        })
        if (res?.data?.success && Array.isArray(res.data.data)){
      ...
      if (results.length === 0) {
        ...
        if (query) {
          const searchRes = await Axios.post('http://localhost:5173/api/product/search-product', {
            search: query,
            limit: 24
          })
  ...
  const fetchProductDetails = async (idParam) => {
    ...
    try {
      ...
      const response = await Axios.post('http://localhost:5173/api/product/get-product-details', {
        productId: idToUse
      });
  ...
  const loadProducts = async () => {
    try {
      ...
      const catResponse = await Axios.get('http://localhost:5173/api/category/get-all');
      ...
      const [dairyRes, fruitsRes] = await Promise.all([
        dairyCat ? Axios.post('http://localhost:5173/api/product/get-product-by-category', {
          id: dairyCat._id
        }) : Promise.resolve({ data: { success: false } }),
        
        fruitsCat ? Axios.post('http://localhost:5173/api/product/get-product-by-category', {
          id: fruitsCat._id
        }) : Promise.resolve({ data: { success: false } })
      ]);
